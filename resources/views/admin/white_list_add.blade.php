...
<div class="main-panel">
  <div class="content-wrapper">
    ...
    <h4 class="card-title text-left">White List Users</h4>
    ...
    <div class="row">
      <div class="col-lg-12 grid-margin stretch-card">
        <div class="card">
          <div class="card-body">
        <a class="btn-style" href="{{route('export')}}">Extract whitelist As Excel sheet</a>

              <a class="btn-style" href="{{url('/admin/whitelist-user-add')}}">All Users</a>
              <!-- <a class="btn-style" href="{{url('/admin/one-thousand-whitelisted-user')}}">One Thousand Winners Users</a> -->
              <!-- <a class="btn-style" href="{{url('/admin/five-hundred-whitelisted-user')}}">Five Hundred Winners Users</a> -->
              <a class="btn-style" href="{{url('/admin/downloadTxt')}}">Extract Wallets ID As txt file</a>
              <a class="btn-style" href="{{url('/admin/attach-file-add')}}">attach file</a>
              <a class="btn-style" href="{{url('/admin/attach-smart-contract')}}">attach smart contract</a>
                ...
                <tbody>
                  ...
                  @foreach($users as $user)
                    @php
                      ...
                      $twitter_id='';
                      $twitter_uname='';
                      $account=DB::table('user_accounts')->where('user_id',$user->uid)->get();
                      //echo"<pre>";
                      //print_r($account);
                      //echo"<pre>";
                    @endphp
                    @foreach($account as $acc)
                      @if($acc->account_type == 'twitter')
                        @php
                          $twitter_id=$acc->account_address;
                          $twitter_uname=$acc->account_title;
                        @endphp
                      @else
                        @php
                          $discord_id=$acc->account_address;
                        @endphp
                      @endif
                    @endforeach
                  <tr>
                  <td>{{$user->name}}</td>
                    ...
                    <td>{{$user->score}}</td>
                    <td>
                     <!-- @if ($user->is_whitelisted == 1) Enable @else Disable  @endif -->
                     <select class="form-control selectbox1" name="is_whitelisted" id="selectbox1" data-id="<?=$user->uid?>">
                        <option value="0" {{ $user->is_whitelisted == 0 ? 'selected' : '' }}>Disable</option>
                        <option value="1" {{ $user->is_whitelisted == 1 ? 'selected' : '' }}>Enable</option>
                        
                    </select>
                    <!-- <select id="selectbox2"></select> -->
                    </td>
                    <td>
                      @if ($user->status == 1) Enable @else Disable  @endif
                    </td>
                    <td>{{$user->enter_rafale}}</td>
                    <td>
                        <div class="task-action-button">
                            <a href="/admin/edit_list/<?=$user->uid?>" class="btn btn-primary task_edit btn-sm">Edit<i class="remove mdi mdi-pencil m-0 ps-1"></i></a>
                            <a href="/admin/delete_list/<?=$user->uid?>" class="btn btn-warning task_delete btn-sm" >Delete<i class="remove mdi mdi-delete m-0 ps-1"></i></a>
                        </div>
                    </td>
                  </tr>
                @endforeach
    ...
    jQuery.ajax({
        type:"GET",
        url : "/admin/whitelist-user-list/"+id,
