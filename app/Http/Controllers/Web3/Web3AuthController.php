...
class Web3AuthController extends Controller
{
    ...
    public function authenticate(Request $request)
    {
        ...
        return redirect('/')->with('message',['status'=>'success','msg'=>'Login Successfully']);
    }
    ...
    public function fortAuth(Request $request)
    {
    //    echo"hello >> ".$_GET['account'];
        ...
        session()->put('login_nonce', $code);
        $users=session()->get('users');
        Log::debug("eth address in request: {$address}");
    ...
    public function disconnect(Request $request)
    {
        auth()->logout();
        session()->forget('login_nonce');
        return redirect('/')->with('message',['status'=>'success','msg'=>'Disconnect Successfully']);
    }
}
